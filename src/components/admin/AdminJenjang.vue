        ...
        <tbody>
          <tr v-for="(item, index) in items">
            ...
            <td :data-label="headers[indexSub]" v-for="(sub, indexSub) in item.data" v-if="item.edit" >
              <!-- edit save -->
              <v-text-field v-if="headers[indexSub] != 'kd_jenjang'"
              ...
              <div v-if="headers[indexSub] == 'kd_jenjang'" style="color:purple;">
                Auto
              </div>
            </td>
  ...
  export default {
    ...
    methods: {
      ...
      save (item) {
        ...
        .then(resp => {
          if (resp === true) {
            let post = {}
            for (let i = 0; i < this.headers.length; i++) {
              if (this.headers[i] !== 'kd_jenjang') {
                post[this.headers[i]] = item.data[i]
              }
            }
            this.$axios.post(this.$baseApi + '/jenjang', post)
            .then(resp => {
              if (resp.data !== null) {
                this.$swal(
                  'Sukses',
        ...
        this.$swal({
          ...
          preConfirm: res => {
            return new Promise((resolve, reject) => {
              this.$axios.delete(this.$baseApi + '/jenjang/' + item.data[0])
              .then(res => {
      ...
      update (item, index) {
        ...
        .then(resp => {
          if (resp === true) {
            ...
            this.$axios.put(this.$baseApi + '/jenjang/' + id, put)
            .then(resp => {
              console.log(resp)
              if (resp.data !== null) {
                this.$swal(
      ...
      get (page = false, limit = false) {
        ...
        this.loading = true
        this.$axios.get(this.$baseApi + `/jenjang`, {params})
        .then(resp => {
          let data = resp.data.jenjang
